...
public class SchemaRewriter {
  ...
  public Schema adaptSchema(Schema original, String matchedPath) {
    ...
    SchemaBuilder builder = SchemaUtil.copySchemaBasics(original);
    for (Field field : original.fields()) {
      String updatedPath =
          matchedPath.isEmpty() ? field.name() : matchedPath + pathDelimiter + field.name();
  ...
  private void adaptField(
      Type decisiveType, SchemaBuilder builder, Field field, String updatedPath) {
  ...
  private void adaptArraySchema(Field field, SchemaBuilder builder, String fieldPath) {
    try {
      if (CipherMode.ENCRYPT == cipherMode) {
            ...
            FieldMode.ELEMENT == fieldMode
                ? SchemaBuilder.array(
                        typeSchemaMapper.getSchemaForPrimitiveType(
                            ...
                            field.schema().valueSchema().isOptional(),
                            cipherMode))
                    .build()
        ...
        Map<String, Object> fieldSpec = extractFieldSpecFromConfig(fieldPath);
      ...
      if (CipherMode.ENCRYPT == cipherMode) {
        ...
        builder.field(
            ...
            FieldMode.ELEMENT == fieldMode
                ? SchemaBuilder.map(
                        typeSchemaMapper.getSchemaForPrimitiveType(
                            field.schema().keySchema().type(),
                            field.schema().keySchema().isOptional(),
                            cipherMode),
                        typeSchemaMapper.getSchemaForPrimitiveType(
                            field.schema().valueSchema().type(),
                            field.schema().valueSchema().isOptional(),
  ...
  private Type derivePrimaryType(Field field, String fieldPath) {
    try {
      if (CipherMode.ENCRYPT == cipherMode) return field.schema().type();
      FieldConfig fc = fieldConfig.get(fieldPath);
      Map<String, Object> fs =
          fc.getSchema()
              .orElseThrow(
                  () ->
                      new DataException(
                          "schema-aware data needs schema spec for "
                              + cipherMode
                              + " but none was given"
                              + " for field path '"
                              + fieldPath
  ...
  private Schema extractAndAdaptArraySchemaFromConfig(
      Map<String, Object> fieldSpec, String fieldPath) {
    Type arrayType = extractTypeFromConfig(fieldSpec, fieldPath);
    if (Type.ARRAY != arrayType) {
      throw new DataException(
          "expected " + Type.ARRAY.getName() + " but found " + arrayType.getName());
    }
    boolean isArrayOptional = extractTypeOptionalFlagFromConfig(fieldSpec);
    Type valueType = extractSubTypeFromConfig(fieldSpec, "valueSchema", null, fieldPath);
    // TODO: value type for array shall support non-primitive types too
    if (!valueType.isPrimitive()) {
      throw new DataException(
          "expected primitive value type for array elements but found " + valueType.name());
    }
    boolean isValueOptional = extractSubTypeOptionalFlagFromConfig(fieldSpec, "valueSchema");
    SchemaBuilder sb =
        SchemaBuilder.array(
  ...
  private Schema extractAndAdaptMapSchemaFromConfig(
      Map<String, Object> fieldSpec, String fieldPath) {
    Type mapType = extractTypeFromConfig(fieldSpec, fieldPath);
    if (Type.MAP != mapType) {
      throw new DataException("expected " + Type.MAP.getName() + " but found " + mapType.getName());
    }
    boolean isMapOptional = extractTypeOptionalFlagFromConfig(fieldSpec);
    Type keyType = extractSubTypeFromConfig(fieldSpec, "keySchema", null, fieldPath);
    boolean isKeyOptional = extractSubTypeOptionalFlagFromConfig(fieldSpec, "keySchema");
    Type valueType = extractSubTypeFromConfig(fieldSpec, "valueSchema", null, fieldPath);
    boolean isValueOptional = extractSubTypeOptionalFlagFromConfig(fieldSpec, "valueSchema");
    // TODO: key + value types for map shall support non-primitive types too
    if (!keyType.isPrimitive() || !valueType.isPrimitive()) {
      throw new DataException(
          "expected primitive types for both map key and map value but found types: "
              + keyType.name()
              + " - "
              + valueType.name());
    }
    SchemaBuilder sb =
        SchemaBuilder.map(
            typeSchemaMapper.getSchemaForPrimitiveType(keyType, isKeyOptional, cipherMode),
            typeSchemaMapper.getSchemaForPrimitiveType(valueType, isValueOptional, cipherMode));
    return isMapOptional ? sb.optional().build() : sb.build();
  }

  @SuppressWarnings("unchecked")
  private Schema extractAndAdaptStructSchemaFromConfig(
      Map<String, Object> fieldSpec, String fieldPath) {
    Type structType = extractTypeFromConfig(fieldSpec, fieldPath);
    if (Type.STRUCT != structType) {
      throw new DataException(
          "expected " + Type.STRUCT.getName() + " but found " + structType.getName());
    }
    boolean isStructOptional = extractTypeOptionalFlagFromConfig(fieldSpec);
    SchemaBuilder structBuilder = SchemaBuilder.struct();
    List<Map<String, Object>> fields =
        Optional.ofNullable((List<Map<String, Object>>) fieldSpec.get("fields"))
            .orElseGet(
                () -> {
                  throw new DataException(
                      Type.STRUCT.getName() + " is missing its mandatory field definitions");
                });
    fields.forEach(
        map -> {
          String nestedFieldName = extractFieldNameFromConfig(map);
          Type nestedFieldType = extractSubTypeFromConfig(map, "schema", null, nestedFieldName);
          boolean isNestedFieldOptional = extractSubTypeOptionalFlagFromConfig(map, "schema");
          String updatedPath = fieldPath + pathDelimiter + nestedFieldName;
          if (Type.ARRAY == nestedFieldType) {
            structBuilder.field(
                nestedFieldName,
                extractAndAdaptArraySchemaFromConfig(
                    (Map<String, Object>) map.get("schema"), updatedPath));
          } else if (Type.MAP == nestedFieldType) {
            structBuilder.field(
                nestedFieldName,
                extractAndAdaptMapSchemaFromConfig(
                    (Map<String, Object>) map.get("schema"), updatedPath));
          } else if (Type.STRUCT == nestedFieldType) {
            structBuilder.field(
                nestedFieldName,
                extractAndAdaptStructSchemaFromConfig(
                    (Map<String, Object>) map.get("schema"), updatedPath));
          } else {
            structBuilder.field(
                nestedFieldName,
                typeSchemaMapper.getSchemaForPrimitiveType(
                    nestedFieldType, isNestedFieldOptional, cipherMode));
          }
        });
  ...
  private Map<String, Object> extractFieldSpecFromConfig(String fieldName) {
    FieldConfig fc = fieldConfig.get(fieldName);
    return fc.getSchema()
        .orElseThrow(
            () ->
                new DataException(
  ...
  @SuppressWarnings("unchecked")
  private Type extractSubTypeFromConfig(
      Map<String, Object> schema, String key, Type expected, String fieldName) {
