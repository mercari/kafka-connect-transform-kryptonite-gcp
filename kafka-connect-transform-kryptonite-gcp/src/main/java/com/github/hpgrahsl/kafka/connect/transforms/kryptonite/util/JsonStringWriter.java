...
public abstract class JsonStringWriter<R extends ConnectRecord<R>> implements Transformation<R> {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(JsonStringWriter.class);

  @Override
  public R apply(R r) {

    Object data = operatingValue(r);

    if (data == null) {
      LOGGER.warn("data was null -> passing it through without SMT processing");
      return null;
    }

    if (!(data instanceof Map)) {
      LOGGER.error("unexpected data type: {}", data.getClass());
      throw new DataException("error: data expected to be of type Map but was " + data.getClass());
    }
    ...
    try {
      ...
      throw new DataException("error: processing the record value failed", e);
    }
  }
