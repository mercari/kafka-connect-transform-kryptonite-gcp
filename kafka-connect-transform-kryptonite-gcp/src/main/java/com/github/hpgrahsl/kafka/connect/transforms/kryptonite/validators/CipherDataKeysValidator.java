...
public class CipherDataKeysValidator implements Validator {
  ...
  static {
    VALID_KEY_LENGTHS.addAll(Arrays.asList(16, 24, 32));
  }
    ...
    try {
      ...
      if (!dataKeyConfig.stream()
        ...
        throw new ConfigException(
            name,
            o,
            "data key specification violation -> invalid key length "
                + "(number of bytes must be one of "
                + VALID_KEY_LENGTHS
                + ")");
      }
      ...
      throw new ConfigException(
          name,
          o,
          "data key specification violation -> "
  ...
  public String toString() {
    return "JSON array holding at least one valid data key config object, "
        + "e.g. ["
        + "{\"name\":\"my-key-id\",\"version\":\"1234\",\"material\":\"dmVyeS1zZWNyZXQta2V5JA==\"}"
        + "]";
  }
}
